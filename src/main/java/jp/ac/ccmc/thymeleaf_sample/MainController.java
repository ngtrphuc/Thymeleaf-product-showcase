...
public class MainController {
    ...
    public String showTopPage(Model model) {
        ...
        model.addAttribute("address", "埼玉県朝霞市");
        model.addAttribute("postalCode", "351-0035");
    ...
    public String showItemPage(@PathVariable("id") String id, Model model) {
        Optional<Map<String, String>> phoneOpt = phones.stream()
                .filter(p -> p.get("id").equals(id))
                .findFirst();

        if (phoneOpt.isPresent()) {
            Map<String, String> phone = phoneOpt.get();
            model.addAttribute("itemId", phone.get("id"));
            model.addAttribute("itemName", phone.get("name"));
            model.addAttribute("price", phone.get("price"));
            model.addAttribute("image", phone.get("image"));
            model.addAttribute("os", phone.get("os"));
            model.addAttribute("ram", phone.get("ram"));
            model.addAttribute("cpu", phone.get("cpu"));
            model.addAttribute("speed", phone.get("speed"));
            model.addAttribute("storage", phone.get("storage"));
            model.addAttribute("size", phone.get("size"));
            model.addAttribute("resolution", phone.get("resolution"));
            model.addAttribute("battery", phone.get("battery"));
            model.addAttribute("charging", phone.get("charging"));
            model.addAttribute("description", phone.get("description"));
        } else {
            model.addAttribute("itemName", "製品が見つかりません");
            ...
            model.addAttribute("resolution", "-");
            model.addAttribute("battery", "-");
            model.addAttribute("charging", "-");
            model.addAttribute("description", "この製品の説明はありません。");
        }
